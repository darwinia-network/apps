...
import { ComponentMap, Props } from '../../../model/param';

import { Account } from './Account';
import { Amount } from './Amount';
import { Balance } from './Balance';
import { BoolParam as Bool } from './Bool';
...
import { OptionDisplay as Option } from './Option';
import { Raw } from './Raw';
...
export const findComponent = (
  ...
  if (!Component) {
    ...
    if (!warnList.includes(type)) {
      warnList.push(type);
      if (error) {
        console.error(`params: findComponent: ${error}`);
      }
      console.info(
